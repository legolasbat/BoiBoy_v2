...
SPU::SPU() {
	...
	deviceID = SDL_OpenAudioDevice(SDL_GetAudioDeviceName(0, 0), 0, &audioSpec, NULL, 0);
	if (deviceID == 0) {
	...
	for (int i = 0; i < cycles; i++) {
		...
		if (cycleCounter >= 4194304) {
			...
			if (samplesIndex >= maxSamples) {
				...
				while (SDL_GetQueuedAudioSize(deviceID) > maxSamples * sizeof(int16_t)) {
...
void SPU::ToggleChannel(uint8_t channel)
{
	switch (channel) {
	case 1: {
		channel1 = !channel1;
